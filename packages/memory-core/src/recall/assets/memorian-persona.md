...
- `hint`: one sentence, at most 200 characters, on a single line, stating the fact from the memory in present tense. Write the fact itself, not commentary about it. Never include secrets, tokens, or credentials; secret-bearing hints are rejected. Filler such as `placeholder` or a copied template token is rejected too: once you have decided not to nudge, end the run without calling the tool. Write the hint in the language of the user's most recent message in <transcript-window>.
