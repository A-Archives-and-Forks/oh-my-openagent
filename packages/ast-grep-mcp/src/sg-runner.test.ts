...
function fixtureRunner(): { directory: string; executable: string } {
  const directory = mkdtempSync(join(tmpdir(), "omo-sg-runner-"));
  directories.push(directory);
  const executable = join(directory, "fake-sg");
...
if (mode.startsWith("exit-1-controlled-")) {
  ...
  process.once("SIGUSR1", () => { clearInterval(hold); process.exitCode = 1; });
  const writeNext = (index) => {
    if (index === writes.length) return fs.writeFileSync(marker + ".ready", "ready");
...
async function runControlledExitOne(directory: string, executable: string, mode: string, name: string) {
  const marker = join(directory, name);
  const ready = waitForFile(directory, `${name}.ready`);
  const running = spawnSgRunner({ sgPath: executable, args: [mode, marker], workdir: directory });
  const outcome = running.then(
    (value) => ({ status: "resolved" as const, value }),
    (error: unknown) => ({ status: "rejected" as const, error }),
  );
  await ready;
  process.kill(Number(readFileSync(`${marker}.pid`, "utf8")), "SIGUSR1");
...
describe("bounded sg NDJSON runner", () => {
  it("#given complete chunked NDJSON #when sg exits #then every record is parsed", async () => {
    ...
    const result = await spawnSgRunner({ sgPath: executable, args: ["complete"], workdir: directory, maxMatches: 10 });
  ...
  it("#given a partial final line #when sg exits #then complete records are salvaged", async () => {
    const { directory, executable } = fixtureRunner();
    const result = await spawnSgRunner({ sgPath: executable, args: ["truncated"], workdir: directory, maxMatches: 10 });
  ...
  it("#given oversized, malformed-only, invalid UTF-8, and empty streams #when read #then failures are classified", async () => {
    const { directory, executable } = fixtureRunner();
    await expect(spawnSgRunner({ sgPath: executable, args: ["oversized"], workdir: directory })).rejects.toMatchObject({ code: "OUTPUT_TOO_LARGE" });
    await expect(spawnSgRunner({ sgPath: executable, args: ["malformed"], workdir: directory })).rejects.toMatchObject({ code: "OUTPUT_PARSE_FAILED" });
    await expect(spawnSgRunner({ sgPath: executable, args: ["invalid-utf8"], workdir: directory })).rejects.toMatchObject({ code: "ENCODING_ERROR" });
    expect((await spawnSgRunner({ sgPath: executable, args: [], workdir: directory })).records).toEqual([]);
  });

  it("#given sg exit 2 #when no internal termination was requested #then the call rejects as SG_FAILED", async () => {
    const { directory, executable } = fixtureRunner();

    await expect(spawnSgRunner({ sgPath: executable, args: ["exit-2"], workdir: directory })).rejects.toMatchObject({
      code: "SG_FAILED",
  ...
  it("#given sg run exit 1 #when no matches were found #then the empty result remains successful", async () => {
    const { directory, executable } = fixtureRunner();
    const result = await spawnSgRunner({ sgPath: executable, args: ["exit-1"], workdir: directory });
  ...
  it("#given sg run exit 1 with an operational diagnostic #when stdout is empty #then the call rejects as SG_FAILED", async () => {
    const { directory, executable } = fixtureRunner();
    const outcome = await runControlledExitOne(directory, executable, "exit-1-controlled-error", "exit-1-error");
  ...
  it("#given an ERROR line after the retained stderr cap #when sg exits 1 #then the call rejects as SG_FAILED", async () => {
    const { directory, executable } = fixtureRunner();
    const outcome = await runControlledExitOne(directory, executable, "exit-1-controlled-flood", "exit-1-flood");
  ...
  it("#given an ERROR prefix split across stderr writes after the cap #when sg exits 1 #then the call rejects as SG_FAILED", async () => {
    const { directory, executable } = fixtureRunner();
    const outcome = await runControlledExitOne(directory, executable, "exit-1-controlled-split", "exit-1-split");
  ...
  it("#given sg run exit 1 with a warning only #when stdout is empty #then it remains a valid no-match", async () => {
    const { directory, executable } = fixtureRunner();
    const result = await spawnSgRunner({ sgPath: executable, args: ["exit-1-warning"], workdir: directory });
  ...
  it("#given more than maxMatches #when the extra record arrives #then sg is terminated with an honest lower bound", async () => {
    const { directory, executable } = fixtureRunner();
    const result = await spawnSgRunner({ sgPath: executable, args: ["many"], workdir: directory, maxMatches: 2 });
  ...
  it("#given excessive stderr #when sg exits #then stderr is capped by UTF-8 bytes", async () => {
    const { directory, executable } = fixtureRunner();
    const result = await spawnSgRunner({ sgPath: executable, args: ["stderr"], workdir: directory });
  ...
  it("#given invalid UTF-8 and a split multibyte boundary #when stderr is decoded #then returned UTF-8 stays within the byte cap", async () => {
    const { directory, executable } = fixtureRunner();
    const invalid = await spawnSgRunner({ sgPath: executable, args: ["stderr-invalid"], workdir: directory });
    const boundary = await spawnSgRunner({ sgPath: executable, args: ["stderr-boundary"], workdir: directory });
  ...
  it("#given a child that ignores SIGTERM #when aborted #then SIGKILL follows the grace period and no process remains", async () => {
    const { directory, executable } = fixtureRunner();
    ...
    const running = spawnSgRunner({ sgPath: executable, args: ["hang", marker], workdir: directory, signal: controller.signal, timeoutMs: 5_000 });
    const outcome = running.then(
      (value) => ({ status: "resolved" as const, value }),
      (error: unknown) => ({ status: "rejected" as const, error }),
    );
  ...
  it("#given a hung child #when the whole-call budget expires #then timeout termination is bounded", async () => {
    const { directory, executable } = fixtureRunner();
    ...
    const running = spawnSgRunner({ sgPath: executable, args: ["hang", marker], workdir: directory, timeoutMs: 500 });
    const outcome = running.then(
      (value) => ({ status: "resolved" as const, value }),
      (error: unknown) => ({ status: "rejected" as const, error }),
    );
