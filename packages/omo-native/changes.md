...
`test/category-coverage.test.ts` (new): the real engine and the real entry over a zai-only and an anthropic-only agent dir (1 and 7 usable, agent dir untouched), a setup plan whose imported key, custom provider and category pin all count, fail-open for doctor and setup (no `categories` row), and the line/row shape. `test/payload.test.ts` requires the new artifact.
