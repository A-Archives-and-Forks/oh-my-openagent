 ...
 * the only writer of the cache: it re-runs the FULL live detection - identical to `omo setup` and
 * `omo doctor` - and atomically rewrites `harness-detect-cache.json` in the canonical agent state
 * directory. Every failure is silent (stdio is ignored by the spawner): a lost refresh only means
 * the next launch refreshes again, and nothing here can ever break a launch.
 */
