...
test("#given a cold member extension #when its module loads #then host utilities and schema builders are not on RPC readiness", async () => {
  ...
  const child = Bun.spawn([process.execPath, "--eval", `
    ...
    console.log(JSON.stringify({ hostUtilities, schemaBuilders, errorCode }));
  `], { cwd: import.meta.dir, stdout: "pipe", stderr: "pipe", timeout: 10000 })
  const [exitCode, stdout, stderr] = await Promise.all([
    child.exited, new Response(child.stdout).text(), new Response(child.stderr).text(),
  ])
  expect({ exitCode, stderr }).toEqual({ exitCode: 0, stderr: "" })
  expect(JSON.parse(stdout)).toEqual({ hostUtilities: [], schemaBuilders: [], errorCode: "missing_env" })
}, 15000)
