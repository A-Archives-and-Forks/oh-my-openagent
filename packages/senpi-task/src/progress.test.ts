...
describe("child task progress", () => {
  test("#given child events #when progress is composed #then activity carries target, model, turn and tool counts", () => {
    ...
    expect(details).toEqual({
      progress: {
        activity: "st_00000001 · quick (kimi-coding/kimi-k3-unlocked:max) · turn 1 (1 tool) · running · 50 tok/s",
        startedAt: 1_000,
      },
      childId: "st_00000001",
  ...
  test("#given a human description #when progress is composed #then the activity leads with what the task is, not its id", () => {
    ...
    expect(progress.details().progress.activity).toBe("Audit the waiting line · quick (kimi-coding/kimi-k3-unlocked:max) · turn 0 · running")
