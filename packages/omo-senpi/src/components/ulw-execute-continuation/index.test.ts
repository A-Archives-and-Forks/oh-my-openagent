...
describe("omo-senpi ulw-execute-continuation", () => {
  it("#given no boulder state #when agent_end fires #then stays quiet", async () => {
    ...
    await pi.dispatch("agent_end", { type: "agent_end", messages: [{ role: "assistant", stopReason: "stop" }] }, eventCtx(root, "qa-s1"))
  ...
  it("#given malformed boulder JSON #when agent_end fires #then no injection and no throw", async () => {
    ...
    const results = await pi.dispatch("agent_end", { type: "agent_end", messages: [{ role: "assistant", stopReason: "stop" }] }, eventCtx(root, "qa-s1"))
  ...
  it("#given missing session manager #when agent_end fires #then skips silently", async () => {
    ...
    await pi.dispatch("agent_end", { type: "agent_end", messages: [{ role: "assistant", stopReason: "stop" }] }, { cwd: root })
