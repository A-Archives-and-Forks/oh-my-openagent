...
import { UNCONFIGURED_POSTHOG_API_KEY, getTelemetryApiKey } from "@oh-my-opencode/telemetry-core"
...
describe("OmO Native product identity", () => {
  test("#given the native product #when config is created #then identity derivation and geoip settings are fixed", () => {
    ...
    expect(OMO_NATIVE_POSTHOG_API_KEY).toBe(UNCONFIGURED_POSTHOG_API_KEY)
