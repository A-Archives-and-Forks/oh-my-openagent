...
| `task-rpc-bridge.ts` / `task-rpc-codec.ts` | `omo.task.updated` snapshot push plus task send/cancel/output over RPC; the codec bounds every field (`st_` id pattern, 32k text caps, 1000 tail lines). Progress-driven pushes coalesce on a 150ms trailing edge (latest-wins, injectable `TaskRpcTimers`) so a burst of child ticks cannot overflow the consumer socket queue; `attach`/`sync` still emit immediately and cancel any pending flush. |
