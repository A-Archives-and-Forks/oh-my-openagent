...
Self-targeting: the caller's identity is read per call, from the execution context's `sessionManager.getSessionId()` (the component's `callerSessionId` option is only the fallback for hosts that don't pass one). Exactly two addresses reach the caller's own thread: the literal `"self"` and the caller's explicit durable id. `"self"` is looked up in the address book by that id, and `caller_context_missing` comes back when the caller's session isn't in the book. Name matching never lands on the caller by accident because fuzzy resolution (`thread_handoff`, `match: "fuzzy"`) filters the caller's entry out of the candidate set before scoring.
