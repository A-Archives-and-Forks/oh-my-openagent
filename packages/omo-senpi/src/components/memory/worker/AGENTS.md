...
| `spawn.ts` / `spawn-supervisor.ts` / `spawn-payload.ts` / `reflection-spawn-input.ts` | Reflection/dream spawn contracts, payload materialization, supervision, and cleanup. `spawn.ts` is a thin re-export surface consumed by memory wiring and tests. Facts and the memorian judge run in-process via senpi-task's InProcessRunner. |
...
| `run-reconciliation.ts` | Startup scan: run directories, process liveness, sentinel waiting, state repair. Retired directories with ledger/terminal timestamps older than the active reservation do not hide a confirmed-dead launcher; historical artifacts stay intact while pending work promotes. |
| `run-temporaries.ts` | Startup cleanup of run/completion JSON temporaries older than 24 hours, including terminal directories. PID-bearing temporaries also require confirmed owner death; fresh files, directories, and live/unknown owners are preserved. |
