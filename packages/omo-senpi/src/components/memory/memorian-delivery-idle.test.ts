...
function sessionHooks(delivery: MemorianDelivery, context: ReturnType<typeof createMemoryIdentityContext>) {
...
describe("memorian accept-time delivery", () => {
  test("#given a running prompt session #when nudges are accepted #then one steer batches every nudge", async () => {
    ...
    expect(sends.messages[0]).toMatchObject({ message: { customType: "omo-memorian:recall", display: false }, options: { deliverAs: "steer" } })
  ...
  test("#given a session without a prompt event #when nudges are accepted #then they stay queued without steering", async () => {
    ...
    const f = await fixture()
    let sends = 0
    const delivery = createMemorianDelivery({ ledgerFor: () => f.ledger, pendingFor: () => f.pending, sendMessage: () => { sends += 1 }, appendEntry: () => {} })
