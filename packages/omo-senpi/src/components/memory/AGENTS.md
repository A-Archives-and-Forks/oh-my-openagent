...
| `recall-wiring.ts` / `memorian-wiring.ts` / `memorian-runner.ts` | Memorian recall gate. `recall-wiring` collects lexical candidates at settle (planner input is USER text only) and injects the gate's pending nudges at `before_agent_start` as one hidden `omo-memorian:recall` message; `memorian-wiring` is the settle/compaction seam (fire-and-forget, drops pending on accepted compaction); `memorian-runner` spawns the quick-pinned judge child with `--tools nudge,read`, `-e` the materialized nudge extension AND `--no-extensions`, under a 5-minute deadline with no queue or failure store. |
...
7. **Recall is a gate, not letta's conversation-bootstrap injection.** There is no bootstrap-time injection and no lexical auto-injection: at settle, a quick-pinned memorian child judges the turn's lexical candidates and answers only through a `nudge` tool, and the validated nudges are injected on the NEXT turn. `/search` remains the manual recall surface (letta's local path already disables AI description generation, C46).
