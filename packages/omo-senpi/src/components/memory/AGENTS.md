...
| `facts-wiring.ts` / `facts-runner.ts` | Durable facts queue (settle-time enqueue, session-start reconcile) + quick-pinned in-process extractor using the shared memory-child helper and one `record_fact` tool; the parent applies the whole batch under the `memory-write` lock. Run-dir artifacts are unchanged, and the ledger records the parent pid. Shutdown enqueues but does not launch facts; the next session reconciles them. |
