...
describe("facts payload byte cap", () => {
  ...
  test("#given a backlog larger than the cap #when a launch runs #then the payload stays within the cap and only shipped endpoints are consumed", async () => {
    ...
    // when: the sandbox seam observes the payload file before the terminal write deletes it.
    const options = runnerOptions(root, identity, queue, "fact", { now: () => NOW })
    let payloadBytes = -1
    const result = await new FactsExtractorRunner({
      ...options,
      sandbox: (args) => {
        payloadBytes = Buffer.byteLength(readFileSync(args.paths.payload, "utf8"), "utf8")
    ...
    const ledger = JSON.parse(await readFile(join(runDir, "ledger.json"), "utf8")) as FactsRunLedger
    const shipped = ledger.queued.map((key) => key.conversationId).sort()
    // Only one bulk conversation fits beside the small one; the newest bulk wins the budget.
    expect(shipped).toEqual(["session-1", "session-3"])
    expect(payloadBytes).toBeGreaterThan(0)
    expect(payloadBytes).toBeLessThanOrEqual(MAX_FACTS_PAYLOAD_BYTES)
    const consumed = JSON.parse(
      await readFile(factsQueuePaths(identity.paths).consumedPath, "utf8"),
    ) as FactsConsumedWatermark
    expect(Object.keys(consumed.consumed).sort()).toEqual(shipped)
  ...
  test("#given a conversation whose older entry is unconsumed #when the cap forbids both #then neither ships and the watermark stays put", async () => {
    ...
    // then: the closed prefix m1+m2 ships; m3 cannot join without exceeding the cap.
    expect(result.status).toBe("committed")
    const runDir = await onlyRunDir(identity)
    const ledger = JSON.parse(await readFile(join(runDir, "ledger.json"), "utf8")) as FactsRunLedger
    expect(ledger.queued.map((key) => key.end_message_id)).toEqual(["m1", "m2"])
    const consumed = JSON.parse(
      await readFile(factsQueuePaths(identity.paths).consumedPath, "utf8"),
    ) as FactsConsumedWatermark
    expect(consumed.consumed["session-1"]?.end_message_id).toBe("m2")
    expect((await queue.listPending()).map((entry) => entry.range.end_message_id)).toEqual(["m3"])
  }, 30_000)
})
