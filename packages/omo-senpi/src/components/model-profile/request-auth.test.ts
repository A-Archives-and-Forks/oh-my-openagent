...
import { AuthStorage, ModelRegistry } from "../../senpi-test-runtime"
...
function drive(config: OmoConfig, registry: InstanceType<typeof ModelRegistry>) {
  ...
  const start = () =>
    pi.dispatch(
      ...
      { type: "session_start", reason: "startup", initialModelProvenance: "settings" },
      {
        mode: "rpc",
        cwd: "/project",
        agentDir: mkdtempSync(join(tmpdir(), "omo-model-profile-real-")),
