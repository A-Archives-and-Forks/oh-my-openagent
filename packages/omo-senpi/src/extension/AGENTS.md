...
| `tool-capture-registry.ts` | Wraps `pi.registerTool` to capture full ToolDefinitions WITH live execute closures (`pi.getAllTools()` returns metadata without closures - plan-reviewer finding). Must wrap before any component registers; lsp registers earlier than task. |
