...
fdIt(
	...
	() => {
		const root = mkdtempSync(
			join(tmpdir(), "omo-senpi-same-size-observation-"),
		);
		try {
...
noFollowIt(
	"#given a same-size in-place overwrite after a protected read #when metadata is verified #then the snapshot fails closed",
	() => {
		...
		try {
			...
			expect(
				isolationState.protectedSnapshotsUntouched(snapshot, snapshot),
			).toBe(false);
		} finally {
			rmSync(root, { recursive: true, force: true });
		}
...
noFollowIt(
	"#given an existence probe would hide inaccessible protected state #when opened directly #then EACCES fails closed",
	() => {
...
noFollowIt(
	"#given ENOENT stat then open success and close failure #when absence races #then FILE_REPLACED remains primary",
	() => {
...
fdIt(
	"#given the root vanishes after its initial identity check #when directory open reports absence #then the snapshot fails closed as replacement",
	() => {
