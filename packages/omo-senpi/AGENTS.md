...
| `src/components/` | Twelve live components: `config-startup`, `ultrawork`, `start-work-continuation`, `ulw-loop`, `todo-continuity`, `fallback-architect`, `comment-checker`, `telemetry`, `lsp`, `codegraph`, `task`, and `config-watch`, plus the `config-resolution` loader helper. |
...
- `todo-continuity`: reads the previous turn's latest `senpi.todo-state` on each interactive input. Open work produces a compact hidden reminder on the idle path or an appended reminder inside a queued prompt, preserving leading commands and images; disabled, extension-sourced, already-tagged, terminal, malformed, and unavailable session state is skipped.
