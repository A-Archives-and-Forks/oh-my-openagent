...
| `src/components/` | Twelve live components: `config-startup`, `ultrawork`, `start-work-continuation`, `ulw-loop`, `todo-fanout-reminder`, `fallback-architect`, `comment-checker`, `telemetry`, `ast-grep`, `lsp`, `task`, and `config-watch`, plus the `config-resolution` loader helper. |
...
| `skills/` | Native Senpi skills authored directly against the Senpi tool surface (not ported from Codex or the shared pool); currently `hyperplan`, `init-deep`, `ultrawork`, `ulw-loop`, and `ulw-research`. `sync-skills.mjs` ships them verbatim. |
