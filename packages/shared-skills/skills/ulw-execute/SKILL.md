...
3. **Keep them current at every moment.** Mark a todo in_progress when its work dispatches and done immediately after its verification passes. Never batch-complete at the end, never execute work that is not a registered todo; discovered work — a pre-existing bug, failing test, stale doc, or wrong guidance met mid-run included — is appended to the plan as a checkbox in the current wave and mirrored as a todo before it runs, and is fixed to the ideal state, never deferred as a follow-up. The todo list, Boulder state, and plan checkboxes must always tell the same story.
