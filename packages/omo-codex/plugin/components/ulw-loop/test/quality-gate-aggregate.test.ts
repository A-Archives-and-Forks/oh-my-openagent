...
describe("aggregated quality gate validation", () => {
	it("#given four independent defects #when validating #then reports all defects in one error", () => {
		const gate = {
			manualQa: {
				by: "main-session", status: "passed", evidence: "placeholder", surfaceEvidence: [], adversarialCases: [],
				artifactRefs: [{ id: "missing", kind: "cli-transcript", description: "artifact", path: "does-not-exist.txt" }],
			},
			gateReview: { by: "category:deep", recommendation: "REJECT", reportPath: "test/fixtures/artifacts/gate-review.md", blockers: [] },
			iteration: { fullRerun: true, status: "passed", rerunCommands: ["test"], evidence: "rerun" },
			criteriaCoverage: { totalCriteria: 1, passCount: 1, originalIntent: "intent", desiredOutcome: "outcome", userOutcomeReview: "review", adversarialClassesCovered: ["none"] },
		};
		try { validateQualityGate(gate, opts); throw new Error("expected validation failure"); }
		catch (error) {
			...
			expect(error.details?.fields).toEqual(expect.arrayContaining([
