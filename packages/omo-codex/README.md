...
The installer copies the built plugin into `~/.codex/plugins/cache/sisyphuslabs/omo/<version>/`, writes the local marketplace snapshot under `~/.codex/.tmp/marketplaces/sisyphuslabs/plugins/omo/`, copies bundled agent TOMLs into `~/.codex/agents/`, enables `omo@sisyphuslabs` in `~/.codex/config.toml`, and registers the `sisyphuslabs` marketplace from the local built cache. It never enables MultiAgentV2 and never writes subagent thread caps: `agents.max_threads` and `features.multi_agent_v2.max_concurrent_threads_per_session` are not inserted, values LazyCodex wrote in earlier releases (`1000`, `16`) are removed so Codex stock defaults apply, and any other user-set value is left as is. If an older config used `[features] multi_agent_v2 = false`, the installer preserves that explicit disable as table-form `enabled = false`.
