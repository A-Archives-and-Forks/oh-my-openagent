...
On npm 11.17.0 with stock settings, an `npm warn allow-scripts` notice does not by itself mean the postinstall was blocked: an isolated install confirmed `bin/senpi-patch.mjs` ran. This does not cover other npm versions or explicit script-blocking policies. Do not install with `--ignore-scripts`; the engine needs the postinstall patches. If your policy blocks the package, review and approve its scripts using npm's approval workflow before reinstalling.
...
Setup also inherits content, not just credentials: MCP servers translate into `mcp.json` (`local`→`stdio`, `remote`→`http`), global skills copy into the agent dir, and a global `AGENTS.md` carries over — all skip-existing and backup-first. Recognized OAuth provider IDs are mapped before printing `Run '/login <provider>' to re-authenticate.`; gateways and unknown providers are reported separately. For config translation (settings, custom providers, keybinds, agents, omo plugin config), preview with `omo migrate --dry-run`, then apply with `omo migrate --yes`.
