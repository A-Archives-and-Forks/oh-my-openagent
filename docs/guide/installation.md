...
| **GPT-6 Luna**    | openai, chatgpt-subscription             | The GPT-6 light tier. Not a default for any agent or category; available as a manual override. Same effort ladder and limits as GPT-6 Sol. `gpt-6-luna-fast` is the Fast-mode variant. |
| **GPT-5.6 Sol**   | openai, chatgpt-subscription, github-copilot, opencode | The predecessor workhorse tier, no longer the recommended model for any agent. It is the only rung of `deep-low` (medium), and the fallback rung under GPT-6 Astra for `ultrabrain` (max) and under GPT-6 Sol for Hephaestus. |
