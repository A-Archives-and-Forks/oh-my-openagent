...
- A rung counts for Recommended or a profile id only when its provider's credentials resolve at session start. A saved login that can no longer be refreshed (for example an expired Claude subscription) is skipped, the walk moves to the next rung, and the notice names the provider with `/login <provider>`. A literal `provider/model` is applied without this check.
