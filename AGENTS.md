...
4. **CONFIRM THE REAL `~/.senpi/agent` WAS NOT TOUCHED** — `drive.mjs` asserts this automatically by comparing a credential digest (`auth.json`, `models.json`, `settings.json`, `trust.json`) before and after. A driver reporting `SKIP` because the `senpi` binary is absent is NOT a pass; say so in the evidence.
