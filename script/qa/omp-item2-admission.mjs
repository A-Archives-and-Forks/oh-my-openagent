...
assert.ok(["host-admission", "host-admission-process"].includes(scenario))
const executionMode = scenario === "host-admission-process" ? "process" : "in-process"
...
const watcher = watch(join(state, "tasks"), () => {
  ...
  if (record.status === "completed") resolveTerminal(record)
})
...
async function respond(body, response) {
  ...
  if (!parent && !worker) { await blocker; send(response, "BLOCKER_DONE"); return }
  if (worker) {
    assert.ok(!names.includes("workflow") && !names.some(name => name.startsWith("task_") || name.startsWith("team_")))
    if (workerStep++ === 0) { send(response, { name: "workpool", args: { op: "yield", results: [{ key: "a", data: { ok: true } }] } }); return }
    workerYieldCode = toolResult(body).error.code
    assert.equal(workerYieldCode, "yield_unavailable")
  ...
  switch (parentStep++) {
    ...
    case 5:
      assert.equal(toolResult(body).items[0].status, "assigned")
...
writeFileSync(join(sandbox.cwd, ".omo", "omo.json"), JSON.stringify({ task: { global_concurrency: 1, default_concurrency: 1, default_execution_mode: executionMode }, categories: { fixture: { model: "fixture/fixture" } }, memory: { enabled: false } }))
...
try {
  ...
  assert.equal(workerStep, 2)
  assert.equal(readdirSync(join(state, "tasks")).filter(name => /^st_.*\.json$/.test(name)).length, 2)
  const pool = readPool()
  assert.equal(pool.status, "cancelled")
  report = { status: "PASS", executionMode, events, pool, workerYieldCode, provider: "local deterministic HTTP fixture", paidProviderCalls: 0,
