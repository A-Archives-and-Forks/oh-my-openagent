...
Retained memory per session on a shared host fell from 74.6 MiB to 3.1 MiB, measured on one machine. Each session used to leave behind a compiled copy of every loaded extension; the host now compiles that code once and reuses it until a source file changes. The public installer now installs OmO Native: `oh-my-openagent install --platform=native` runs the install, which it had no target for before. Users who installed the OpenCode plugin months ago can see that Native exists from inside the terminal UI, including a toast and a `/native` command. Disk used by saved task-graph history, on a 170 MB copy of one project, fell from 170 MB to 38 MB. Listing 710 saved runs fell from 473 ms median to 3.4 ms.

### Added

**`oh-my-openagent install --platform=native` performs the OmO Native install. The public installer had no Native target before this release.** `native` is in `install --help` and the picker beside OpenCode, Codex and Both. It runs `bun add -g omo-ai@beta` when bun is on PATH, or `npm i -g omo-ai@beta` if it is not, then points you at `omo setup`; on failure it prints the command and the reason. A `--platform=native-dev` path remains for people developing this repository ([#8618](https://github.com/code-yeongyu/oh-my-openagent/issues/8618)).

**An OpenCode session can show that OmO Native exists, from inside the terminal UI.** The only pointer used to print at install time, which npm hides by default and which scrolls away for everyone else. A throttled toast on session start now names OmO Native and the install command, and `/native` in the command palette opens a dialog: install prints the command for you to run, or you can open the guide, be reminded in a week, or stop asking. The toast stays off when Native is already installed, when `native-edition-nudge` is in `disabled_hooks`, in a child session, and when the toast cannot record itself - a showing that cannot be remembered is skipped, so it cannot nag every session. Automatic showings stop after four, with widening gaps, and at most once per process. ([#8619](https://github.com/code-yeongyu/oh-my-openagent/issues/8619))
...
The runtime underneath Native is still the senpi engine, and is still called senpi. The `senpi` command, `@code-yeongyu/senpi`, `SENPI_CODING_AGENT_DIR` and the telemetry identifiers are unchanged.
...
Work lands in `unspecified-high` when no specialist category fits and the job is big, so that category absorbs a large share of delegated turns. Its chain led with Astra at `high`, which put the most expensive reasoning model on the most generic lane.
...
**The `quick` category drops its Kimi HighSpeed model, and the explore and librarian agents pick that model up with thinking turned off.** ([#8616](https://github.com/code-yeongyu/oh-my-openagent/issues/8616))
...
**Normalizing a tool call's arguments no longer rewrites the assistant message the model produced.** A helper that shortens the eval tool's summary, and a helper that rewrites the edit tool's `edits` field, used to change that message in place. On the `claude-sdk-oauth` lane that was the usual cause of `Session continuity lost - resent the full conversation (assistant_rewritten)`. They now run on a detached copy. The 80-character eval summary limit is unchanged: it is enforced on the rendered line (senpi [#1472](https://github.com/code-yeongyu/senpi/issues/1472)).
...
**Opening a second window on a conversation that is already running in Claude Code no longer drops the id used to continue that conversation.** The notice printed at startup used to drop that id, so the next turn re-sent the entire conversation. Notices, user messages and tool results after the last committed assistant keep the id. A later assistant message, a summary, or an explicit invalidation still discards it. If Claude Code says that id is missing, it is dropped so the next turn does not ask for it again (senpi [#1964](https://github.com/code-yeongyu/senpi/issues/1964), [#1958](https://github.com/code-yeongyu/senpi/issues/1958), [#1973](https://github.com/code-yeongyu/senpi/issues/1973)).
...
**A resumed task graph no longer shows nodes as running when nothing is running them.** ([#8657](https://github.com/code-yeongyu/oh-my-openagent/issues/8657))

Resuming a session took back every graph node whose child task still said `running`, without asking whether this process still held that child. A child task whose parent process died is kept as `running` on purpose, so it can be reopened from its transcript later - but a graph node waiting on one waits forever, because the run folds a node only when its child settles here. The node therefore stayed `running` in the run's saved state through restart after restart, with the work behind it blocked, and the widget kept counting hours on children that had died hours ago.
...
**Task-graph run history is pruned on the advertised 7-day retention.** The cleanup function existed, was tested, and was called from nowhere, so checkpoints, event logs, results and keys accumulated forever. A long-lived project directory held 711 checkpoints, the oldest 25 days old against a 7-day policy. Cleanup now runs once when a graph session starts, after start returns so a stale file cannot fail the session, and indexes the keys and locks directories once per pass. A paused run is kept if the process that owns it is still alive. Against a copy of a 170 MB on-disk history directory the pass cut 10,623 files to 1,779 and 170 MB to 38 MB ([#8651](https://github.com/code-yeongyu/oh-my-openagent/issues/8651)).
...
**A failed turn in a delegated task is no longer counted as a turn.** When a provider error ends an assistant turn, that turn now lands in a new `failed_turns` stat instead of inflating `turns`, and its usage - typically an all-zero block the provider sends alongside the error - contributes no tokens, no cost and no generation time. A run that never produced a successful turn reports token and cost coverage as `unavailable` and omits the cost field entirely, instead of claiming `turns: 6` for six consecutive failures. A successful turn that cost $0 keeps reporting a cost of 0, and a failure re-anchors the generation window so the next successful turn's throughput is measured from the failure, not from spawn. The live task row now tells the same story: it reads `starting` until the first successful turn lands - no phantom `turn 0`, no cost token - shows `failed N` with the verb `retrying` while provider attempts keep failing, and returns to `running` only after a real turn. Both the terminal status line and the background task row draw their cost and turn figures from one shared builder, so the two wordings cannot drift apart again. ([#8627](https://github.com/code-yeongyu/oh-my-openagent/issues/8627))

**The Codex plugin again blocks its quality-gate reviewer from starting without a manual-QA file, and again caps no-progress reviewer spawns.** That check is ulw-loop. Renaming the reviewer agents to `omo-native-*` left the Codex-side ulw-loop guard - the check that refuses the gate reviewer unless a manual-QA file is on disk, and that caps no-progress spawns - matching only the retired `omo-senpi-*` spellings. Because the resolver rewrites a leftover `omo-senpi-*` name to `omo-native-*` before the guard sees it, the guard received a name it did not recognize and treated the spawn as ordinary work: the reviewer could start with no manual-QA file, and the no-progress cap stopped counting. Both checks apply again, and either spelling is recognized, so nothing that named the old reviewer breaks. The denial message and the spawn counter now name the reviewer that actually ran ([#8630](https://github.com/code-yeongyu/oh-my-openagent/issues/8630)).
